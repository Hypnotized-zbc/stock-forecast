...
- **排行榜**：沪深A股 涨幅/跌幅/成交额/换手率 四大榜单，点击行直达行情分析
...
| `/api/leaderboard?kind=` | 沪深A股排行榜（kind: up/down/amount/turnover；东财→新浪→磁盘缓存三级，当日缓存优先，启动预取） |
| `/api/fflow?secid=&days=` | 个股历史资金流（主力/超大/大/中/小单净流入，10~120 日；东财→新浪→磁盘缓存三级，当日缓存优先） |
